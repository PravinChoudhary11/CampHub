        ...
        <div className={`rounded-2xl border p-4 sm:p-6 ${darkMode ? 'bg-gray-950/60 border-gray-900' : 'bg-gray-50 border-gray-200'}`}>
          ...
          <form className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            <div className="sm:col-span-2">
              ...
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Scientific Calculator" className={`w-full px-3 py-2.5 rounded-lg border ${inputBg}`} />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={category} onChange={(e) => setCategory(e.target.value)} className={`w-full pl-9 pr-3 py-2.5 rounded-lg border ${inputBg}`}>
            ...
            <div>
              <label className={`block text-xs font-medium mb-1 ${labelClr}`}>Available From (optional)</label>
              <div className="relative">
                <Calendar className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="date" value={availableFrom} onChange={(e) => setAvailableFrom(e.target.value)} className={`w-full pl-9 pr-3 py-2.5 rounded-lg border ${inputBg}`} />
              </div>
            </div>

            <div className="sm:col-span-2">
              <label className={`block text-xs font-medium mb-1 ${labelClr}`}>Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Key details, specs, condition, included accessories..." className={`w-full px-3 py-2.5 rounded-lg border ${inputBg}`} />
            </div>
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              <button type="button" className="px-4 py-2.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-medium">Publish listing</button>
              <button type="reset" onClick={() => { setTitle(""); setPrice(""); setCategory("electronics"); setCondition("like-new"); setLocation(""); setAvailableFrom(""); setDescription(""); setContacts([{ id: 1, type: 'mobile', value: '' }]); setImages([]); setPreviews([]); }} className={`px-3 py-2.5 rounded-lg border text-sm ${darkMode ? 'border-gray-700 text-gray-200' : 'border-gray-300 text-gray-800'}`}>Reset</button>
            </div>
          </form>
        </div>
      </main>
